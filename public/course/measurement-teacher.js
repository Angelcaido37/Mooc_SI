(()=>{
  ...
  function bindHub(){
    ...
    document.querySelector("#pilotConfigForm")?.addEventListener("submit",async event=>{
      ...
      try{await NEXUS_AUTH.savePilotConfig({startDate:data.startDate,groupName:String(data.groupName||"").trim()});notify("Calendario del piloto actualizado.");}
