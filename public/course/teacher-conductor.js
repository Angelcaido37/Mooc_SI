(()=>{
  ...
  function list(sessions){
    return `<header class="page-head"><span class="eyebrow">MODO CONDUCCIÓN DE CLASE</span><h1>26 sesiones ejecutables, minuto a minuto</h1><p>Cada instrucción enlaza el recurso necesario: diapositiva, lectura, técnica, video, actividad, respuesta esperada o plan alternativo. Puede alternarlos sin perder el punto de la sesión.</p></header>
  ...
  function phaseView(session,state){
    ...
    return `<section class="conductor-live" data-conductor-live><header class="conductor-live-head"><div><span class="eyebrow">MOMENTO ${index+1} DE ${session.teacherScript.length} · MINUTOS ${start}–${end}</span><h2>${esc(phase.title)}</h2><p>${state.status==="not-started"?"Revise la preparación y comience cuando el grupo esté listo.":"Mantenga visible sólo lo necesario para este momento."}</p></div><div class="conductor-clock-wrap"><span id="conductorClock" class="conductor-clock ${remaining<=60?"urgent":""}">${formatClock(remaining)}</span><small>restantes de ${minutes} min</small></div></header>
      ...
      <div class="n19-action-dock"><span>RECURSOS DISPONIBLES EXACTAMENTE EN ESTE MOMENTO</span><div class="n19-action-buttons">${(phase.n19Actions||[]).map(a=>`<button type="button" data-n19-resource="${esc(a.kind)}" data-session="${session.id}" data-phase="${index}" data-slide="${a.slide??0}">${esc(a.label)}</button>`).join("")}</div></div>
