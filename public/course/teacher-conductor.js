(()=>{
  ...
  function view(session){
    ...
    const state=current;
    if(state.status==="completed")queueMicrotask(()=>{const actions=$(".conductor-completed .button-row");if(actions&&!actions.querySelector("[data-classroom-session]"))actions.insertAdjacentHTML("afterbegin",`<button class="btn btn-primary" data-classroom-session="${esc(session.id)}">Publicar actividad en Classroom</button>`);});
