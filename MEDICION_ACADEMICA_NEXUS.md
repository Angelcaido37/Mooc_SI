# Medición académica semestral integrada · NEXUS
...
La fecha de inicio se configura en **Portal docente → Medición y resultados → Calendario académico**. NEXUS calcula la semana vigente y abre únicamente los instrumentos correspondientes.
...
Para publicar la plataforma:

1. sustituya la carpeta completa `public/course`;
2. publique el archivo raíz `firestore.rules`;
3. recargue la aplicación hasta que aparezca la identidad **Plataforma académica**;
4. configure la fecha de inicio desde el portal docente.

Las rutas de datos son `coursework/nexusPilotConfig` para el calendario, `progress/{uid}.measurementResponses` para instrumentos estudiantiles y `teacherUsage/{uid}.measurementResponses` para la evaluación docente. El primer identificador se conserva únicamente por compatibilidad interna y no se muestra a los usuarios. Las reglas limitan al estudiante a su propio registro y reservan las consultas agregadas para cuentas docentes.
